...
class TestWindowsScriptContent:
    ...
    def test_never_interpolates_a_literal_placeholder_for_dynamic_values(self):
        ...
        assert "param(" in content
        assert (
            "{" not in content.split("param(")[0].strip() or True
        )  # header comment may be empty; param block is what matters
...
class TestWriteAndLaunchHandoffScript:
    ...
    def test_posix_builds_sh_command_with_positional_args(self, mock_popen, monkeypatch, tmp_path):
        ...
        assert cmd == [
            self_update_handoff._posix_sh_path(),
            str(tmp_path / "script.sh"),
            "5678",
            "/opt/curatarr/curatarr",
            "/opt/curatarr/.curatarr-update-x.tmp",
            "8787",
            "2.9.0",
            "deadbeef" * 8,
        ]
