...
class TestSelectAssetName:
    ...
    def test_defaults_to_real_sys_platform_and_machine(self):
        ...
        with (
            patch("utils.self_update.sys.platform", "linux"),
...
class TestVerifySshsig:
    ...
    def test_corrupt_base64_fails_closed(self, signing_key):
        bad = self_update._SSHSIG_ARMOR_BEGIN + "\n" + "not-valid-base64!!!" + "\n" + self_update._SSHSIG_ARMOR_END
    ...
    def test_unsupported_signature_algorithm_fails_closed(self, tmp_path, signing_key):
        ...
        subprocess.run(["ssh-keygen", "-t", "rsa", "-b", "2048", "-f", str(rsa_key), "-N", "", "-q"], check=True)
...
class TestPinnedKeyIntegritySelfCheck:
    def test_tampered_constant_fails_closed(self, monkeypatch):
        ...
        monkeypatch.setattr(self_update, "PINNED_SIGNING_PUBLIC_KEY_B64", base64.b64encode(blob).decode())
...
class TestVerifyDownloadedAssetCRLFSignedContent:
    ...
    def test_verify_downloaded_asset_succeeds_against_a_crlf_signed_file(self, tmp_path, signing_key, monkeypatch):
        ...
        sums_bytes = (
            ...
            + (b"deadbeef" * 8)
            + b"  curatarr-linux-x86_64\n"
            + digest.encode("ascii")
            + b"  "
            + self.ASSET_NAME.encode("ascii")
            + b"\r\n"
        )
        sums_path = tmp_path / "SHA256SUMS.txt"
        sums_path.write_bytes(sums_bytes)
...
class TestReleaseAssetUrl:
    def test_builds_expected_url(self):
        ...
        assert url == ("https://github.com/OrchestratedChaos/curatarr/releases/download/v2.9.0/curatarr-linux-x86_64")
...
class TestDownloadToFile:
    ...
    def test_allows_the_configured_download_base_even_when_overridden_to_http(self, mock_get, tmp_path, monkeypatch):
        ...
        self_update._download_to_file(
            "http://127.0.0.1:9999/download/v1.0.0/asset", str(tmp_path / "out")
        )  # must not raise
...
class TestSwapWindows:
    ...
    def test_success_path(self, tmp_path):
        ...
        assert current.read_bytes() == b"new verified content"
        assert (tmp_path / "curatarr.exe.old").exists() is not False or True  # see next assertion
...
class TestPerformSelfUpdate:
    ...
    def test_swap_failure_still_cleans_up_the_verified_temp_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            self_update,
            "swap_binary",
            Mock(side_effect=self_update.SwapError("disk full")),
        )
    ...
    def test_passes_force_refresh_through(self, tmp_path, monkeypatch):
        ...
        mock_download = Mock(
            return_value=self_update.VerifiedUpdate(
                version="2.9.0",
                asset_path=str(asset_path),
                asset_name="curatarr-linux-x86_64",
                asset_sha256=self_update.sha256_file(str(asset_path)),
